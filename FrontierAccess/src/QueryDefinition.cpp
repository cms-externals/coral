...
coral::FrontierAccess::QueryDefinition::QueryDefinition( const coral::FrontierAccess::ISessionProperties& properties, const std::string& tableName )
  : m_properties( properties ),
    m_singleTable( true ),
    m_sqlFragment( "" ),
    m_inputData( 0 ),
...
const coral::FrontierAccess::ISessionProperties& coral::FrontierAccess::QueryDefinition::sessionProperties() const
{
  return m_properties;
}
...
{
  if ( ! m_sqlFragment.empty() )
    throw coral::QueryExecutedException( m_properties.domainProperties().service()->name(),"IQueryDefinition::defineSubQuery" );

  if ( m_singleTable ) {
    return m_subQueries.insert( std::make_pair( alias, coral::FrontierAccess::QueryDefinition( m_properties, m_tableList.begin()->first ) ) ).first->second;
  }
  else {
    return m_subQueries.insert( std::make_pair( alias, coral::FrontierAccess::QueryDefinition( m_properties ) ) ).first->second;
  }
}
...
coral::IQueryDefinition& coral::FrontierAccess::QueryDefinition::applySetOperation( coral::IQueryDefinition::SetOperation opetationType )
{
  ...
  if ( m_singleTable )
    m_setQuery.first = new coral::FrontierAccess::QueryDefinition( m_properties, m_tableList.begin()->first );
  else
    m_setQuery.first = new coral::FrontierAccess::QueryDefinition( m_properties );
...
void coral::FrontierAccess::QueryDefinition::process()
{
  ...
  for ( std::vector< std::pair< std::string, std::string > >::const_iterator iTable = m_tableList.begin(); iTable != m_tableList.end(); ++iTable )
  {
    ...
    if( !aliasName.empty() )
    {
      if ( m_subQueries.find( aliasName ) != m_subQueries.end() || m_properties.schema().existsTable( aliasName ) || m_properties.schema().existsView( aliasName ) )
        os << "\"" << aliasName << "\"";
      else
        os << "\"" << aliasName << "\"";
