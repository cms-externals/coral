...
coral::PyCoral::IQueryDefinition_Type()
{
  static PyMethodDef IQueryDefinition_Methods[] = {
    { (char*) "setDistinct", (PyCFunction) IQueryDefinition_setDistinct, METH_NOARGS,
      (char*) "Requires a distinct selection." },
    { (char*) "addToOutputList", (PyCFunction) IQueryDefinition_addToOutputList, METH_VARARGS,
      (char*) "Appends an expression to the output list." },
    { (char*) "addToTableList", (PyCFunction) IQueryDefinition_addToTableList, METH_VARARGS,
      (char*) "Appends a table name in the table selection list (the FROM part of the query)." },
    { (char*) "defineSubQuery", (PyCFunction) IQueryDefinition_defineSubQuery, METH_O,
      (char*) "Defines a subquery. The specified name should be used in a subsequent call to addToTableList." },
    { (char*) "setCondition", (PyCFunction) IQueryDefinition_setCondition, METH_VARARGS,
      (char*) "Defines the condition to the query (the WHERE clause)." },
    { (char*) "groupBy", (PyCFunction) IQueryDefinition_groupBy, METH_O,
      (char*) "Appends a GROUP BY clause in the query ." },
    { (char*) "addToOrderList", (PyCFunction) IQueryDefinition_addToOrderList, METH_O,
      (char*) "Appends an expression to the ordering list (the ORDER clause) ." },
    { (char*) "limitReturnedRows", (PyCFunction) IQueryDefinition_limitReturnedRows, METH_VARARGS,
      (char*) "Instructs the server to send only up to maxRows rows at the result of the query starting from the offset row." },
    { (char*) "applySetOperation", (PyCFunction) IQueryDefinition_applySetOperation, METH_VARARGS,
...
IQueryDefinition_defineSubQuery( PyObject* self, PyObject* args )
{
  ...
  try {
    ...
    PyObject* c_object = PyCObject_FromVoidPtr( theQueryDefinition,0 );
...
IQueryDefinition_applySetOperation( PyObject* self, PyObject* args )
{
  ...
  try {
    ...
    PyObject* c_object = PyCObject_FromVoidPtr( theSetOperation,0 );
    PyObject* temp = Py_BuildValue((char*)"OO", py_this, c_object );
    bool ok = ( ob->ob_type->tp_init( (PyObject*) ob,temp,0)==0);
    Py_DECREF(temp);
    Py_DECREF( c_object );
