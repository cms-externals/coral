...
coral::PyCoral::IWebCacheControl_Type()
{
  static PyMethodDef IWebCacheControl_Methods[] = {
    { (char*) "refreshSchemaInfo", (PyCFunction) IWebCacheControl_refreshSchemaInfo, METH_VARARGS, (char*) "Instructs the RDBMS backend that all the tables within the schema \n specified by the physical or logical connection should be refreshed, in case they are accessed." },
    { (char*) "refreshTable", (PyCFunction) IWebCacheControl_refreshTable, METH_VARARGS,
    ...
    { (char*) "webCacheInfo", (PyCFunction) IWebCacheControl_webCacheInfo, METH_VARARGS,
      (char*) "Returns the web cache information for a schema given the corresponding physical or logical connection.  " },
    { (char*) "compressionLevel", (PyCFunction) IWebCacheControl_compressionLevel, METH_NOARGS,
      (char*) "Returns the previous compression level. " },
    { (char*) "setCompressionLevel", (PyCFunction) IWebCacheControl_setCompressionLevel, METH_O,
      (char*) "Sets the compression level for data transfer, 0 - off, 1 - fast, 5 - default, 9 - maximum" },
    { (char*) "setProxyList", (PyCFunction) IWebCacheControl_setProxyList, METH_VARARGS,
...
IWebCacheControl_init( PyObject* self, PyObject* args, PyObject* /*kwds*/ )
{
  ...
  if ( !PyArg_ParseTuple( args, (char*)"OO",
                          &(py_this->parent),
                          &c_object ) ) return -1;
  py_this->object = static_cast<coral::IWebCacheControl*>( PyCObject_AsVoidPtr( c_object ) );
...
IWebCacheControl_webCacheInfo( PyObject* self, PyObject* args)
{
  ...
  try {
    ...
    PyObject* c_object = PyCObject_FromVoidPtr( webCacheInfo,0 );
