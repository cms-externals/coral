#ifndef CORALCOMMON_SEARCHPATH_H
#define CORALCOMMON_SEARCHPATH_H 1
...
namespace coral {
  ...
  class SearchPath
  {
    ...
    typedef std::list<boost::filesystem::path>                  DirList;
  public:
...
# endif
    ...
    void                        push_front (const boost::filesystem::path& dir);
    void                        push_back (const boost::filesystem::path& dir);
    iterator    insert (iterator pos, const boost::filesystem::path& dir);
    ...
    void                        remove (const boost::filesystem::path& dir);
    void                        reverse (void);
    ...
    expand (const boost::filesystem::path& name) const;
    const_iterator              search (const boost::filesystem::path& name) const;
    const_iterator              search (const_iterator from, const boost::filesystem::path& name) const;
...
# endif // HAVE_MEMBER_TEMPLATES
  ...
  inline void
  SearchPath::push_front (const boost::filesystem::path& dir)
  { m_list.push_front (dir); }

  /** Add a directory @a dir to the back of the search path. */
  inline void
  SearchPath::push_back (const boost::filesystem::path& dir)
  ...
  inline SearchPath::iterator
  SearchPath::insert (iterator pos, const boost::filesystem::path& dir)
  ...
  inline void
  SearchPath::remove (const boost::filesystem::path& dir)
  { m_list.remove (dir); }
  ...
  inline SearchPath::const_iterator
  SearchPath::search (const boost::filesystem::path& name) const
  { return search (begin (), name); }

} // namespace coral
#endif // CORALCOMMON_SEARCHPATH_H
