...
#define ASSERT(x) {; }
  ...
  SearchPath::search (const_iterator from, const boost::filesystem::path& name) const
  {
    ASSERT (! name.empty ());
    ASSERT (! name.isAbsolute ());
    for (const_iterator last = end ();
         from != last && ! boost::filesystem::exists( (*from/name).string() );
         ++from)
      ;
  ...
  SearchPath::expand (const boost::filesystem::path& name) const
  {
