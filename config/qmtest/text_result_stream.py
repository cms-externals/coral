...
class TextResultStream(FileResultStream):
    """A (CORAL-modified) 'TextResultStream' displays test results textually.
    ...
    def WriteResult(self, result):
        ...
        if ( self.format == "full"
             or ( self.format == "brief" and
                  ( result.GetOutcome() != Result.UNTESTED or # See task #18784
                    "QMTEST_DISABLE_PATCH_TASK18784" in os.environ ) and
                  result.GetOutcome() != Result.PASS ) ):
    ...
    def Summarize(self):
        ...
        if self.format in ("full", "brief", "batch"):
            ...
            elif "QMTEST_DISABLE_PATCH_TASK21088" not in os.environ:
                self._DisplayHeading("TESTS THAT DID NOT PASS (EXCLUDING UNTESTED TESTS)") # See task #21088
            else:
    ...
    def _SummarizeTestStats(self):
        ...
        # Display AA nightlies info (see task #21216 and bug #86463)
        if "QMTEST_DISABLE_PATCH_TASK21216" not in os.environ:
            self.file.write("\n")
            self._DisplayHeading("ENVIRONMENT")
            for env in "CMTCONFIG", "LCG_NGT_SLT_NAME", "LCG_NGT_DAY_NAME", "CORAL_AUTH_PATH", "CORAL_DBLOOKUP_PATH", "TNS_ADMIN":
    ...
    def _SummarizeResults(self, results):
        ...
        for result in results:
            if ( result.GetOutcome() != Result.UNTESTED or # See task #21088
                 "QMTEST_DISABLE_PATCH_TASK21088" in os.environ ):
                self._DisplayResult(result, self.format)
            if ( self.format == "batch" and
                 result.GetOutcome() != Result.UNTESTED ): # See task #18784
                self._DisplayAnnotations(result)
